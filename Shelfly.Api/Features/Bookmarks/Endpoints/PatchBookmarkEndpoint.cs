...
public static class PatchBookmarkEndpoint
{
    ...
    public static async Task<IResult> Handle(
        ...
        Guid id,
        BookmarkPatchDto request,
        ILogger logger = null!,
        CancellationToken cancellationToken = default)
    {
        using Activity? activity = ActivitySource.StartActivity("Bookmarks.Patch");
        ...
        BookmarkResponseDto? bookmark = await bookmarkService.PatchBookmarkAsync(userId.Value, id, request, cancellationToken);
        ...
        logger.LogInformation("Patched bookmark {BookmarkId} for user {UserId}", id, userId);
