...
public static class DeleteBookmarkEndpoint
{
    ...
    {
        using Activity? activity = ActivitySource.StartActivity("Bookmarks.Delete");
        ...
        bool deleted = await bookmarkService.DeleteBookmarkAsync(userId.Value, id, cancellationToken);
        ...
        logger.LogInformation("Deleted bookmark {BookmarkId} for user {UserId}", id, userId);
