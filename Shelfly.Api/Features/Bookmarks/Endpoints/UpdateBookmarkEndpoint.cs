...
public static class UpdateBookmarkEndpoint
{
    ...
    {
        using Activity? activity = ActivitySource.StartActivity("Bookmarks.Update");
        ...
        BookmarkResponseDto? bookmark = await bookmarkService.UpdateBookmarkAsync(userId.Value, id, request, cancellationToken);
        ...
        logger.LogInformation("Updated bookmark {BookmarkId} for user {UserId}", id, userId);
