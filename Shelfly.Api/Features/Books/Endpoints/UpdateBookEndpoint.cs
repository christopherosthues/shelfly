...
public static class UpdateBookEndpoint
{
    ...
    {
        using Activity? activity = ActivitySource.StartActivity("Books.Update");
        ...
        BookResponseDto? book = await bookService.UpdateBookAsync(userId.Value, id, request, cancellationToken);
        ...
        logger.LogInformation("Updated book {BookId} for user {UserId}", id, userId);
