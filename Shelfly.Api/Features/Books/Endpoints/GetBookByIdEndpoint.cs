...
public static class GetBookByIdEndpoint
{
    ...
    {
        using Activity? activity = ActivitySource.StartActivity("Books.GetById");
        ...
        BookResponseDto? book = await bookService.GetBookByIdAsync(userId.Value, id, cancellationToken);
        ...
        logger.LogInformation("Retrieved book {BookId} for user {UserId}", id, userId);
